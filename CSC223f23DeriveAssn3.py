...
# Due Date:      Tuesday 11/21/2023
# Course:     CSC223 Fall 2023
# Professor Name: D. Parson
# Assignment: #3
#
#   Student NOTE: 
#       I completed the blanks in the above comments as directed.
#       I also completed VisitorClass(object), where I used examples
#           from partialBindings.py to flesh out the class using
#           the syntax template and followed the pseudocode comments
#       I then completed delta(Nstring) using the closure function
#           example in partialBindings.py and the pseudocode comments
#           provided by the professor.
#       Next I ensured the program can read the original csv file
#           to process it according to the instructions. Each row
#           and column was accounted for and sorted by year and month.
#       Finally, I ensured the program can write out to a
#           separate csv file according to the instructions.
#
...
class VisitorClass(object):
    ...
    def __init__(self, Nstring):
        ...
        self.history = []
    def visit(self, cell): # so-called 'visitor' object-oriented pattern
        self.history.append(cell)
        while len(self.history) > self.N:
            self.history.pop(0)
        roundedHistoryMean = round(stats.mean(self.history),2)
        return roundedHistoryMean
...
if __name__ == '__main__':  # Called from command line
    ...
    incsvreader = csv.reader(indatafile)
    inheader = incsvreader.__next__()
    indataset = [row for row in incsvreader]
    indatafile.close()
    yearcol = inheader.index('year')
    monthcol = inheader.index('month')
    def getkey(row):
        return row[yearcol], row[monthcol]
    indataset.sort(key=getkey)
